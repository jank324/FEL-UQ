...
class Model:
    """
    This is my custom class for all QR model training and evaluation. Useage is
    demonstrated in the QR notebooks. Most important is the scaling dictionary. The
    model architecture is hard coded.
    """

    def __init__(
        self, scaling_dict=None, NAME=None, PATH=None, QUANTILE=0.5, input_dimension=76
    ):
        self.name = NAME
        self.path = PATH
    ...
    def load(self):
        ...
        self.model.compile(
            loss=lambda y_true, y_pred: self.quantile_regression(y_true, y_pred),
            optimizer="adam",
        )
    ...
    def quantile_regression(self, y_true, y_pred, sample_weight=None):
        ...
        loss = tf.reduce_mean(
            tf.maximum(self.quantile * error, (self.quantile - 1) * error), axis=-1
        )
        return loss
    ...
    def train(self, inputs, outputs, epochs=5000, batch_size=4096, saving=True):
        ...
        mc = tf.keras.callbacks.ModelCheckpoint(
            self.path + self.name + "best_model_checkpoint.h5",
            monitor="val_loss",
            mode="min",
            save_best_only=True,
        )
        ...
        self.model.fit(
            ...
            verbose=0,
            callbacks=[mc, es, bf.plot_losses],
        )
        stop = time.time()
        ...
        self.X_test_fullscale = bf.unscale_data(
            X_test, self.xoffsets, self.xscales, -1, 1
        )
        self.Y_test_fullscale = bf.unscale_data(
            Y_test, self.yoffset, self.yscale, -1, 1
        )
        print("Training Complete")

    def train_on_split_data(
        self, input_dict, output_dict, epochs=5000, batch_size=4096, saving=True
    ):
...
def perform_quad_scan(column, inputs, outputs, med, ub, lb, plotting=True):
    """
    Temporary "quad scan."
    Isn't really a scan, just sorts on quad values and plots all of the predictions. The
    remaining inputs VARY, so this isn't a quad scan.
    """
    ...
    if plotting:
        plot_quad_scan(
            column, inps, ub_pred[:, 0], lb_pred[:, 0], median_pred[:, 0], meas
        )
        plot_quad_scan_persample(
            column, inps, ub_pred[:, 0], lb_pred[:, 0], median_pred[:, 0], meas
        )
...
def calc_mse_and_mae(meas, pred):
    """
    Calculated the mean squared error (MSE) and mean absolute error (MAE) for a 1-d
    array.
    """
...
def plot_interpolation_predictions(
    ub_pred, lb_pred, median_pred, meas, outputs, remaining
):
    n = len(ub_pred[:, 0])
    plt.figure(figsize=(20, 8))
    plt.plot(ub_pred[:, 0], color=colors[2], alpha=0.5, label="97.5% Quantile")
    ...
    plt.plot(
        ...
        color=colors[-2],
        alpha=0.75,
        label="Measured Data, Removed from Training",
    )
    plt.plot(
        remaining,
        outputs[remaining],
        "x",
        color=colors[3],
        label="Measured Data, Available for Training",
    )
    plt.plot(median_pred[:, 0], ".", color=colors[1], label="Median Prediction")
    plt.fill_between(
        np.arange(n), ub_pred[:, 0], lb_pred[:, 0], color=colors[2], alpha=0.5
    )
    plt.xlabel("Sample Number")
    plt.ylabel("Pulse Energy (mJ)")
    plt.legend()
...
def plot_quad_scan(column, inps, ub_pred, lb_pred, median_pred, meas):
    ...
    plt.plot(
        inps[:, column], median_pred, ".", color=colors[1], label="Median Prediction"
    )
...
def plot_quad_scan_persample(column, inps, ub_pred, lb_pred, median_pred, meas):
    ...
    plt.fill_between(
        np.arange(len(ub_pred)), ub_pred, lb_pred, color=colors[2], alpha=0.5
    )
    plt.legend()
    plt.show()
...
def plot_individual_points(ub_pred, lb_pred, median_pred, meas, cols=3, rows=5):
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            axs[i, j].vlines(
                0, ub_pred[ind, 0], lb_pred[ind, 0], color=colors[0], alpha=0.25
            )
