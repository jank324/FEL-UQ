...
class Model(nn.Module):
    """
    This is my custom class for all BNN model training and evaluation. Useage is
    demonstrated in the notebooks. Most important is the scaling dictionary. The model
    ...
    def train_on_split_data(
        self, input_dict, output_dict, epochs=300, batch_size=4096, saving=True
    ):
        ...
        self.dataloader_train = torch.utils.data.DataLoader(
            ds_train, batch_size=2048, shuffle=True
        )

        ds_test = torch.utils.data.TensorDataset(X_test, Y_test)
        self.dataloader_test = torch.utils.data.DataLoader(
            ds_test, batch_size=2048, shuffle=True
        )
        ...
        for epoch in range(epochs):
            for i, (datapoints, labels) in enumerate(self.dataloader_train):
                ...
                loss = self.regressor.sample_elbo(
                    inputs=datapoints.to(self.device),
                ...
                iteration += 1
                if iteration % 100 == 0:
                    ic_acc, under_ci_upper, over_ci_lower = self.evaluate_regression(
                        self.regressor,
                    ...
                    print(
                        "CI acc: {:.4f}, CI upper acc: {:.4f}, CI lower acc: {:.4f}"
                        .format(ic_acc, under_ci_upper, over_ci_lower)
                    )
